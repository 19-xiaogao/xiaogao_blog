...
const BlogDetail: React.FC<IProps> = (props) => {
  ...
  const [errorInfo, setErrorInfo] =
    useState<string>("~认真和用心是一种态度, 感谢支持~");
  ...
  useEffect(() => {
    (async () => {
      const { success, data } = await get_blogComment({
        ...
        <div className={Styles.comment_user_list} key={item.id}>
          <div className={Styles.describe}>
            ...
            <div className={Styles.describe_right}>
              ...
              <span>{moment(item.createTime).format("HH:MM YYYY-MM-DD")}</span>
            </div>
          </div>
  ...
  const verifyResponse = async (flg: boolean) => {
    ...
    setCommentEmail("");
    setContext("");
    const { success: commentSuccess, data } = await get_blogComment({
      id: props.blogDetail.id,
      pageNo: 1,
      pageSize: pageSize,
    });
        ...
        <section className={Styles.comment_section}>
          <div className={Styles.comment_form}>
            ...
            <textarea
              ...
              name='context'
              value={context}
              onChange={textareaInputContext}></textarea>
          ...
          <div className={Styles.more}>
            ...
            {total === comment.length ? (
              <div className={Styles.bottomTest}>到底了</div>
            ) : loadingMore ? (
              <LoadingDom />
            ) : (
              <div className={Styles.loadingBtn} onClick={loadMore}>
                加载更多{" "}
              </div>
            )}
          </div>
